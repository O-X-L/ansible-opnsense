...
class HaproxyAction(BaseModule):
    ...
    FIELDS_TYPING = {
        'str': ['name', 'description', 'fcgi_pass_header', 'fcgi_set_param', 'http_request_auth', 'http_request_redirect',
                'http_request_use_service', 'http_request_add_header_name',
                ...
                'tcp_response_inspect_delay', 'custom_rule'],
        'int': ['http_response_set_status_code'],
        'list': ['linked_acls'],
        'select': ['test_type', 'operator', 'type', 'http_request_set_var_scope', 'http_response_set_var_scope',
                   'use_backend', 'use_server', 'http_request_lua', 'http_response_lua', 'tcp_request_content_lua',
                   'tcp_response_content_lua', 'map_use_backend_file', 'map_use_backend_default']
    }
